...
def pred():
    ...
    city = request.form['city'] if request.method == 'POST' else 'Lahore'

    # API request to get weather data
    url = f'https://api.openweathermap.org/data/2.5/weather?q={city}&appid=openweathermap-api-key'
    ...
    # API_KEY = 'AIzaSyAXfuwl4jbYjIiV91p5QYIZIROYvvKpBro'
    # SEARCH_ENGINE_ID = '9073967eeeDBc48ec'
...
# sending report and alert messages notifications
# Twilio credentials
account_sid = 'api'
auth_token = 'Token '
...
def send_report_message(message):
    # Send SMS
    client.messages.create(to="+received twilio massage enter number ", from_=twilio_phone_number, body=message)

    # Send WhatsApp message
    client.messages.create(
        from_=twilio_phone_number,
        body=message,
        to='+received twilio massage enter number'
    )
